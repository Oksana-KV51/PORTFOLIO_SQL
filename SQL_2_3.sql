...
SELECT
  ...
  CASE
    ...
    WHEN gender = 'муж' and EXTRACT(YEAR FROM AGE(CAST(date_of_birth AS date))) < 30 THEN 'Мужчины младше 30'
    WHEN gender = 'муж' AND EXTRACT(YEAR FROM AGE(CAST(date_of_birth AS date))) BETWEEN 30 AND 45 THEN 'Мужчины 30-45'
    WHEN gender = 'муж' AND EXTRACT(YEAR FROM AGE(CAST(date_of_birth AS date))) > 45 THEN 'Мужчины 45+'
    WHEN gender = 'жен' AND EXTRACT(YEAR FROM AGE(CAST(date_of_birth AS date))) < 30 THEN 'Женщины младше 30'
    WHEN gender = 'жен' AND EXTRACT(YEAR FROM AGE(CAST(date_of_birth AS date))) BETWEEN 30 AND 45 THEN 'Женщины 30-45'
    WHEN gender = 'жен' AND EXTRACT(YEAR FROM AGE(CAST(date_of_birth AS date))) > 45 THEN 'Женщины 45+'
    ELSE 'Другая группа' END AS customer_group
FROM
  customers
),
customer_groups AS (
